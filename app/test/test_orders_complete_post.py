...
class CouriersIdPatch(unittest.TestCase):
    ...
    def test_orders_complete_post(self):
        ...
        for i in range(test_count):
            test_data, ans_data = get_test_and_answer("orders_complete_post_simple", i)
            ...
            for command in test_data["commands"]:
                if "complete" in command["route"]:
                    ...
                    timings[command["data"]["courier_id"]] = (
                        timings[command["data"]["courier_id"]][0],
                        timings[command["data"]["courier_id"]][0] + datetime.timedelta(minutes=2)
                    )
